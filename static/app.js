...
const WidgetRegistry = {
  ...
  create(name, ctx) {
    if (!this.types[name]) return null;
    return this.types[name](ctx);
  },
};
...
function renderSchema(name) {
  ...
  gridCols = schema.cols;
  gridRows = schema.rows;
  ...
  schema.cells.forEach((c, idx) => {
    ...
    ph.textContent = "Trascina un widget qui"; //meow
    cell.appendChild(ph);
...
function placeWidget(cell, type) {
  ...
  if (!widget) {
    cell.innerHTML = "<div class='placeholder'>Widget non disponibile</div>";
    return;
  }
  ...
  // decide se il widget è "small" (1x1 o quadrato) o "long" (≥2 colonne o ≥2 righe)
  const colSpan = cell.style.gridColumn.match(/span\s+(\d+)/);
  const rowSpan = cell.style.gridRow.match(/span\s+(\d+)/);
  const cols = colSpan ? parseInt(colSpan[1]) : 1;
  const rows = rowSpan ? parseInt(rowSpan[1]) : 1;
  if (cols <= 1 && rows <= 1) {
...
btnTallLeft.addEventListener("click", () => {
  renderSchema("tall-left");
  saveLayout();
});
btnTallRight.addEventListener("click", () => {
  renderSchema("tall-right");
  saveLayout();
});
btnWideTop.addEventListener("click", () => {
  renderSchema("wide-top");
  saveLayout();
});
btnWideBottom.addEventListener("click", () => {
  renderSchema("wide-bottom");
  saveLayout();
});
...
function loadLayout() {
  ...
  try {
    ...
    const schemaName =
      layout.schema && PRESETS[layout.schema] ? layout.schema : "2x2";
